...
function getFailedFlagFromState() {
  const hardenerFailed = core.getState('hardenerFailed')
  if (isFailed) {
    return hardenerFailed === 'true'
  }
  return isFailed
}
...
function checkSelfHostedRunner() {
  ...
  if (runnerName && !runnerName.startsWith('GitHub')) {
    setFailedFlagInState()
    if (graceful) {
...
function init(platform, arch) {
  ...
  if (getPostFlagFromState()) {
    if (getFailedFlagFromState()) {
      core.info('Skipping post action as hardener failed')
      return
    }
